-- =====================================================
-- Final Complete Setup - Run this in SQL Editor
-- =====================================================

-- 1. Drop all policies first (to avoid conflicts)
...
DROP POLICY IF EXISTS "Public read org units" ON organizational_units;
DROP POLICY IF EXISTS "Public read invitations" ON invitations;
DROP POLICY IF EXISTS "Public read permissions" ON resource_permissions;
DROP POLICY IF EXISTS "Public read audit logs" ON audit_logs;
DROP POLICY IF EXISTS "Authenticated users can update their org" ON organizations;
...
-- 8. Insert Sample Resource Permissions
...
-- 10. Re-enable RLS with simple policies (for development)
...
-- Simple development policies (allow all for authenticated users)
CREATE POLICY "Allow all for authenticated" ON organizations
    FOR ALL USING (true);

CREATE POLICY "Allow all for authenticated" ON organizational_units
    FOR ALL USING (true);
...
CREATE POLICY "Allow all for authenticated" ON resource_permissions
    FOR ALL USING (true);
