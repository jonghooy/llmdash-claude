/**
 * AuthService - Refactored with Repository Pattern
 * Maintains the same API surface while using the AuthRepository abstraction
 */
...
const { getAuthRepository } = require('~/server/repositories/AuthRepositoryFactory');
...
const logoutUser = async (req, refreshToken) => {
  try {
    ...
    const session = await authRepo.findSession({ userId: userId, refreshToken });

    if (session) {
      try {
        await authRepo.deleteSession({ sessionId: session._id });
      } catch (deleteErr) {
...
const verifyEmail = async (req) => {
  ...
  const user = await authRepo.findUser({ email: decodedEmail }, 'email _id emailVerified');
  ...
  let emailVerificationData = await authRepo.findToken({ email: decodedEmail });
  ...
  const updatedUser = await authRepo.updateUser(emailVerificationData.userId, { emailVerified: true });
  ...
  await authRepo.deleteTokens({ token: emailVerificationData.token });
...
const registerUser = async (user, additionalData = {}) => {
  ...
  let newUserId;
  try {
    const existingUser = await authRepo.findUser({ email }, 'email _id');
    ...
    const isFirstRegisteredUser = (await authRepo.countUsers()) === 0;
    ...
    const newUserData = {
      ...
      // Add approval status if required
    ...
    const newUser = await authRepo.createUser(newUserData, appConfig.balance, disableTTL, true);
    ...
    if (emailEnabled && !newUser.emailVerified) {
      ...
      await authRepo.updateUser(newUserId, { emailVerified: true });
    }
    ...
    if (newUserId) {
      const result = await authRepo.deleteUserById(newUserId);
      logger.warn(
...
const requestPasswordReset = async (req) => {
  ...
  const user = await authRepo.findUser({ email }, 'email _id');
  const emailEnabled = checkEmailConfig();
  ...
  await authRepo.deleteTokens({ userId: user._id });
...
const resetPassword = async (userId, token, password) => {
  let passwordResetToken = await authRepo.findToken({
    userId,
  });
  ...
  const user = await authRepo.updateUser(userId, { password: hash });
  ...
  await authRepo.deleteTokens({ token: passwordResetToken.token });
...
const setAuthTokens = async (userId, res, sessionId = null) => {
  try {
    ...
    const token = await authRepo.generateToken(user);
    ...
    if (sessionId) {
      session = await authRepo.findSession({ sessionId: sessionId }, { lean: false });
      refreshTokenExpires = session.expiration.getTime();
      refreshToken = await authRepo.generateRefreshToken(session);
    } else {
      const result = await authRepo.createSession(userId);
...
const resendVerificationEmail = async (req) => {
  try {
    const { email } = req.body;
    await authRepo.deleteTokens(email);
    const user = await authRepo.findUser({ email }, 'email _id name');
